...
This week, we will explore the relationship of Haskell to Java, and, in the
...
and returns nothing, a use-case seemingly omitted from the `java.util.function`
...
Remember `stringFitsFormat`? That function is intended to recognize strings
like "1a2aa" and "3aaa", where the string consists of pairs of a one-digit
number followed by that many "a"s. Here is its implementation:
