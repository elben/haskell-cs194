...
type AccountList = TArray AccountNum Account

deposit :: AccountList -> AccountNum -> Integer -> STM ()
deposit accts acct = withdraw accts acct . negate

withdraw :: AccountList -> AccountNum -> Integer -> STM ()
...
transfer accts amt from to = atomically $ do
...
mkAccounts = atomically $ newArray (1, numAccounts) 0

sumAccounts :: AccountList -> IO Integer
sumAccounts accts = sum `liftM` atomically (getElems accts)
